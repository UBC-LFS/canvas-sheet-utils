...
const editFile = (actionFunct, data) => {
  fs.readFile('client_secret.json', (err, content) => {
    if (err) return console.log('Error loading client secret file:', err)
    // Authorize a client with credentials, then call the Google Sheets API.
    authorize(JSON.parse(content), actionFunct, data)
  })
}
 ...
 */
function listCourses (auth, range = '') {
...
function listCanvasCourseCopies (auth, range = '') {
  const sheets = google.sheets({version: 'v4', auth})
  sheets.spreadsheets.values.get({
    spreadsheetId: '1Df4TF67exj3_6H9B53sIp5lbRS_3FBt011sGRJ-zVAg',
    range: '2018W1!A2:N',
...
function listXlisted (auth, range = '') {
  const sheets = google.sheets({version: 'v4', auth})
  sheets.spreadsheets.values.get({
    spreadsheetId: '1Df4TF67exj3_6H9B53sIp5lbRS_3FBt011sGRJ-zVAg',
    range: '2018W1!A2:N',
...
function testFunct (auth, outputArray) {
  const sheets = google.sheets({version: 'v4', auth})
  sheets.spreadsheets.values.get({
    spreadsheetId: '1Df4TF67exj3_6H9B53sIp5lbRS_3FBt011sGRJ-zVAg',
    range: '2018W1!A2:N',
    ...
    if (rows.length) {
      rows.map((row) => {
        if (row[13] === 'Y') {
          outputArray.push(`${row[2]}, ${row[3]}, ${row[5]}, ${row[8]}`)
        }
      })
      console.log('Copied')
    } else {
